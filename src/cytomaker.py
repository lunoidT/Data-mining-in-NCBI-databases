...
def help():
    ...
    print("-c <int> Filtering based on amount of gene connections per PubMedID")
    print("-n <str> Filtering based on if user-prompted string (part of gene name) appears in connection")
    print("-u <int> Filtering based on weighed sum of connections")
    print()
...
def parsecommand():
    ...
    while len(sys.argv) > 1:
        ...
        # -n is followed by a name which takes up multiple words (spaces) which must all be removed
            ...
            if len(name) == 0:
                usage("Please provide a name after -n.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "cytofile_" + file_options["tax_id"] + ".csv" in os.listdir("cytofiles"): 
            # O(k), where k is amount of connections in existing file
            ...
            # Overall runtime for this section when no previous file: O(1+n^2*m+k). After simplifying: O(n^2*m)
            # This applies for both quickfilter and no quickfilter.
        ...
        if filtering > 0:
            ...
            # Overall runtime for this section: O(k)
